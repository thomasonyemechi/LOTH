...
define("DB_USER1", "thehillt_hilluser");//enter your database username
define("DB_PASS1", "light.com");   //databse password
define("DB_NAME1", "thehillt_hilldb");//database name


$db2 = new mysqli(DB_SERVER1, DB_USER1, DB_PASS1, DB_NAME1);
...
class Chat{
	...
	public function chatUsers($userid){
		global $pro;
		$cat = $pro->userName($pro->Uid(), 'category');
		$sqlQuery =$db2->query("SELECT * FROM user WHERE id!='$userid' AND category='$cat' ");
		return  $this->getData($sqlQuery);
	}
	/// get details
	...
	public function ggetUserDetails($userid, $col=''){
		global $db;
		$sql =$db2->query("SELECT * FROM user WHERE id = '$userid'");
